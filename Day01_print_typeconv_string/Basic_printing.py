...
# print(f"my name is {name}") # Used F-String (format string)
# print(f"i am {age} years old")
# print(datetime.now()) # It return current time
...
async def scrape_review_page(page) -> tuple[list[dict], bool]:
    ...
    for i in range(reviews_count):
        ...
        try:
            ...
            # 📅 DATEbb
            r_date = (await review.locator("div.typo-0").first.inner_text()).strip()
                            ...
                            if retries < max_retries:
                                ...
                                await request_queue.add_request(
                                    Request.from_url(
                                        url,
                                        user_data={
                                            **request.user_data,   # 🔥 KEEP ALL DATA
                                            'retries': retries + 1
                                        }
                                    ),
                                    forefront=True,
