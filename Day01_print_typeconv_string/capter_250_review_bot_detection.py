...
async def detect_human_check(page) -> bool:
    try:
        # Structure-based check (more robust)
        if await page.locator('div.cb-c input[type="checkbox"]').count() > 0:
            Actor.log.warning("⚠️ Human verification checkbox detected (input[type=checkbox]).")
            return True
        
        # Fast text-based check (your snippet shows this text)
        if await page.locator("text=Verify you are human").count() > 0:
            Actor.log.warning("⚠️ Human verification checkbox detected (text).")
            return True
        ...
        if "Verify you are human" in html:
            Actor.log.warning("⚠️ Human verification detected (HTML match).")
            return True

        return False
    except Exception as e:
        Actor.log.warning(f"Human-check detection error: {e}")
        return False
...
async def wait_for_cloudflare(page, timeout_s=45) -> bool:
    ...
    while asyncio.get_event_loop().time() < deadline:
        ...
        if(detect_human_check):
            button = await page.locator('div.cb-c input[type="checkbox"]')
            await button.click
